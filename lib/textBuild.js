    ...
    for(let i=0; i<key.length; i++){
        ...
        if(skip){
            ...
            for(j=0; j<rows.length; j++){
                ...
                if( (key[i] === `manager_id`) && (row[key[i]])){
                    let manager = row[`first_name`] + ' ' + row[`last_name`]
                    //console.log(manager)
                    if(manager.length > max){
                        max = manager.length
                    }
                }
                else if(row[key[i]]){
            ...
            for(j=0; j<rows.length; j++){
                ...
                if(row[key[i]]){
                    if(row[key[i]].toString().length < max){
                        ...
                        if(key[i] === `manager_id`) text[j+2] = text[j+2] + row[`first_name`] + ' ' +row[`last_name`]
                        else text[j+2] = text[j+2] + row[key[i]].toString();
